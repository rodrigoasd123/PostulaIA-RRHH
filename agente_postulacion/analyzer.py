...
def _sentences(pages: list[PageText]):
    for page in pages:
        for raw in SENTENCE_RE.split(page.text):
            ...
            # Las bases suelen usar filas o viñetas muy breves (p. ej. "Contrato: 3 meses").
            if 12 <= len(sentence) <= 650:
                yield page.page, sentence


def _collect(pages: list[PageText], pattern: re.Pattern, limit: int = 12) -> list[Evidence]:
    matches, seen = [], set()
...
def analyze_document(pages: list[PageText]) -> DocumentAnalysis:
    ...
    requirements = _collect(pages, PATTERNS["requirements"])
    dates = _collect(pages, re.compile(PATTERNS["dates"].pattern + "|" + DATE_VALUE.pattern, re.I))
    conditions = _collect(pages, PATTERNS["conditions"])
    exclusions = _collect(pages, PATTERNS["exclusions"])
    alerts = _collect(pages, PATTERNS["alerts"])
    summary_parts = []
    if requirements:
